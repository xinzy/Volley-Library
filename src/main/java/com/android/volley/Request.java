...
public abstract class Request<T> implements Comparable<Request<T>> {
	...
	/**
	 * Creates a new request with the given URL and error listener. Note that the normal response listener is not provided here as delivery of
	 * responses is provided by subclasses, who have a better idea of how to deliver an already-parsed response.
	 *
	 */
	public Request(String url, RequestCallback<T> callback) {
	 ...
	 * response.
	 */
	public Request(int method, String url, RequestCallback<T> callback) {
		mMethod = method;
		mUrl = url;
	...
	public Request<T> setTimeout(int timeout) {
		return setPolicy(timeout, DefaultRetryPolicy.DEFAULT_MAX_RETRIES);
	}

	public Request<T> setRetryTimes(int times) {
		return setPolicy(DefaultRetryPolicy.DEFAULT_TIMEOUT_MS, times);
	}
	
	public Request<T> setPolicy(int timeout, int times) {
		RetryPolicy policy = new DefaultRetryPolicy(timeout, times, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
	...
	public Request<T> setRequestCallback(RequestCallback<T> requestCallback) {
	...
	/**
	 * Returns a list of extra HTTP headers to go along with this request. Can throw {@link AuthFailureError} as authentication may be required to
	...
	public Request<T> setHeader(Map<String, String> headers) {
		this.headers = headers;
		return this;
	}

	/**
	 * Returns a Map of parameters to be used for a POST or PUT request. Can throw {@link AuthFailureError} as authentication may be required to
	 * provide these values.
	 *
	 * <p>
	...
	public Request<T> setParamsEncoding(String encoding) {
	...
	private byte[] encodeParameters(Map<String, String> params, String paramsEncoding) {
		...
		try {
			for (Map.Entry<String, String> entry : params.entrySet()) {
                ...
                encodedParams.append(entry.getValue() == null ? "" : URLEncoder.encode(entry.getValue(), paramsEncoding));
				encodedParams.append('&');
			}
	...
	/**
	 * Returns the socket timeout in milliseconds per retry attempt. (This value can be changed per retry attempt if a backoff is specified via
	 * backoffTimeout()). If there are no retry attempts remaining, this will cause delivery of a {@link TimeoutError} error.
	 */
	...
	public void deliverStart() {
		if (mRequestCallback != null) {
			mRequestCallback.onStart(this);
		}
	}
	
	public void deliverCancelled() {
		if (mRequestCallback != null) {
			mRequestCallback.onCancelled(this);
		}
	}
	
	public void deliverLoading(long total, long current) {
		if (mRequestCallback != null) {
			mRequestCallback.onLoading(this, total, current);;
		}
	}
	
	public void deliverUserdCache() {
		if (mRequestCallback != null) {
			mRequestCallback.onUsedCache(this);
		}
	}
