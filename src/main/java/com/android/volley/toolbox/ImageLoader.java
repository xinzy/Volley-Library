 ...
 * {@link ImageLoader#getImageListener(ImageView, int, int)}. Note that all function calls to this class must be made from the main thead, and all
...
public class ImageLoader {
	 ...
	 */
	public static abstract class ImageListener extends RequestCallback<Void> {
		/**
		 ...
		 */
		public abstract void onResponse(ImageContainer response, boolean isImmediate);
	}
	 ...
	 *
	 * The ImageContainer will contain either the specified default bitmap or the loaded bitmap. If the default was returned, the {@link ImageLoader}
	...
	protected Request<Bitmap> makeImageRequest(String requestUrl, int maxWidth, int maxHeight, ScaleType scaleType, final String cacheKey) {
		RequestCallback<Bitmap> callback = new RequestCallback<Bitmap>() {
			@Override
		...
		return new ImageRequest(requestUrl, maxWidth, maxHeight, scaleType, Config.RGB_565, callback);
	}
	...
	private static String getCacheKey(String url, int maxWidth, int maxHeight, ScaleType scaleType) {
		return new StringBuilder(url.length() + 12).append("#W").append(maxWidth).append("#H").append(maxHeight).append("#S")
