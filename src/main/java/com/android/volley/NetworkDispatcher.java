 ...
 *
 * Requests added to the specified queue are processed from the network via a specified {@link Network} interface. Responses are committed to cache,
 * if eligible, using a specified {@link Cache} interface. Valid responses and errors are posted back to the caller via a {@link ResponseDelivery}.
 */
		...
		while (true) {
			...
			try {
				...
				if (request.isCanceled()) {
					...
					mDelivery.postCancelled(request);
					continue;
				}
